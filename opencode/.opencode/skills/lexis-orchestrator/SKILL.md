...
| `narrative-summarizer`| Extraction (Per-Chapter) | `read`, `write`, `glob` | `notes/<name>.summary.txt`, `notes/<name>.challenges.md` |
| `local-lexicographer` | Extraction (Per-Chapter) | `read`, `write`, `glob` | `notes/<name>.lexicon.json` |
...
| `stray-phrase-detector`| Production (Validation Stage)| `grep`, `bash`, `read` | `notes/<name>.stray_report.json` |
