...
- **Positive-Constraint override (AUTHORITATIVE — do this first):** If `notes/POSITIVE_CONSTRAINTS.md` exists, treat its locked term table (SECTION A) as the source of truth. For any glossary term whose `SOURCE` matches a PCD row, set the glossary `translation` to the row's `USE_ONLY` value exactly, and NEVER set it to a `NEVER_USE` value. Record the PCD's intent in `usage_notes` (e.g. "PCD-locked: use 電子桌; never 課桌 — this is a futuristic datapad, not a school desk"). A human/operator authored these to defeat the mid-tier model's tendency to pick a generic or literal rendering; do not "improve" or second-guess them.
