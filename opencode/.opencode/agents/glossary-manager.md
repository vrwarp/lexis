...
- **Never list a colliding form (MANDATORY — the most dangerous corruption class).** A `never_variants` entry must be a **complete, name-specific token of ≥ 2 characters** that does NOT occur inside ordinary, unrelated words. NEVER list:
  - a **single character** (it will collide with common words — e.g. listing `森` as a wrong form of the name "Shen → 申" makes a blind swap turn 森林 *forest* into the garbage 申林, and Anderson 安德森 into 安德申). One-syllable names that map to a single hanzi must rely on context (native-critique / the LLM fixer), NOT a deterministic swap — set `never_variants: []` for them.
  - a form that is a **substring of another name's canonical or of a valid in-world compound** (e.g. bare `薩拉曼德` when the army is `火蜥蜴軍團` — `薩拉曼德` also begins `薩拉曼德大門`, so swapping the stem mangles the compound). List the full token (`薩拉曼德軍團`), not the stem.
  - a common dictionary word. When in doubt, OMIT the variant and let the Name-Variant detector merely *flag* it for review — a missed flag is recoverable; a corrupted deliverable is not.
