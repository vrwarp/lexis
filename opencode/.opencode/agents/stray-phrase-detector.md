...
0b. **Leaked Meta-Text / Long-Run Source Scan (MANDATORY, deterministic, HARD FAIL).** A mid-tier translator/finalizer sometimes emits its own reasoning ("Let me now produce the final translation…", "I have all the source material…") or leaves a whole clause untranslated; a per-token Latin scan can miss a long English sentence that the model "explains itself" in. Run BOTH, regardless of the stray-source mode:
   ...
   - **Meta-phrase scan (case-insensitive):** `grep -niE "let me|i have|i will|i'll|i am going to|the prompt|source material|the exemplar|the critique|remediation|as an ai|here is|here's the|final translation|final translated|translated text|the draft|the source text|add particle|preserves granularity|no merges"`.
   - **Editorial-annotation scan (catches mostly-target-script pipeline notes the Latin scan misses):** a leaked style-guide / critique note is often *mostly target-script* with only a verb of instruction, so the long-run scan won't see it. Flag any line that is a markdown bullet or numbered item (`^\s*[-*]\s` or `^\s*\d+\.\s`) **and** contains an instruction arrow (`→` / `->`) or an English imperative (`Add|Use|Preserve|Ensure|Note|Continue|Match|Avoid|Render`); any line wrapped in `**…**` that is a header rather than emphasis (e.g. `**FINAL TRANSLATED TEXT (zh-TW):**`); and any standalone `---` divider that is not part of the source. Real narrative prose is none of these. *(Observed leak this scan must catch: `2. **「…」** → Add particle 啦 for intimacy/urgency` and `- Preserves granularity (no merges/collapses)`.)*
...
11. **Name-Variant Check (BLOCKING, deterministic).** Proper-noun romanization drift is the dominant consistency failure on a per-scene mid-tier pipeline (the same character transliterated two or three ways — including a slip/sign block rendered differently from running prose). For every `proper_noun` in the master glossary that carries a `romanization` + canonical hanzi (and optional `never_variants`): grep the draft for the canonical form AND for each `never_variants` form. If a name appears in MORE THAN ONE hanzi form anywhere in the file, list every variant with line numbers under `## Name Variant`, naming the canonical form. Emit a Repair Block per non-canonical hit (token swap `FIND_EXACTLY: <variant> / REPLACE_WITH: <canonical>`) — you are a copier, not a translator. This is part of the `ISSUES_FOUND` set. (Structured blocks like a transfer slip "COMMANDER BONZO MADRID" are the usual offenders — scan the whole file, not just prose.)
   ...
   - **Collision guard (MANDATORY — only emit a swap that is provably safe to apply blindly):** emit a `FIND_EXACTLY / REPLACE_WITH` swap ONLY when the variant is a **complete name token of ≥ 2 characters** that does not occur inside any unrelated word. Do NOT auto-swap (a) a **single character** (it collides with ordinary words — e.g. `森`/forest is also the name-syllable Shen; blind-swapping it produces 申林 from 森林), (b) a form that is a **prefix of a longer token actually present in the draft** (e.g. don't swap `薩拉曼德軍` when the draft contains `薩拉曼德軍團` — you'd leave a dangling 團 → `火蜥蜴軍團團`; verify the character AFTER each match is not part of a longer name token), or (c) a stem that appears in valid compounds. For any such risky variant, do NOT emit a Repair Block — instead list it under `## Name Variant (review)` for `native-critique` / the operator to resolve with context. A missed flag is recoverable; a blind swap that corrupts the deliverable is not.
