...
8. **Negation-Parity Check (ADVISORY).** Count negation markers in source (`not|n't|never|no|without`) vs draft (`不|沒|別|未|無|莫`) with grep; if the draft has materially fewer, emit `## Negation Deficit` naming the likely source paragraph. This flags dropped/inverted negation (a meaning error). ADVISORY/REVIEW only — never auto-"fix" by inserting 不 (it would land in the wrong place); route to native-critique/operator.
9. **Particle Gate (for register).** On scenes whose register is `DIALOGUE` or `INTERIORITY` (or any chapter with ≥4 dialogue lines), grep for TW colloquial terminal particles `[啦喔嘛吧欸哦呢囉]`. If a dialogue/interiority scene has ZERO, emit `## Particle Absent` listing the scene id(s) — a signal the register came out flat/formal (the silent 好友/老友 failure). The orchestrator uses this to trigger a gated particle-retranslation; you only report presence/absence.
