...
Output only the translated text. Match the source's paragraph boundaries one-to-one (the omission audit maps paragraph to paragraph), but within a paragraph you may freely merge, split, or reorder sentences where that yields more natural prose — sentence-level mirroring is not required.
