...
1. **Apply Repair Blocks (zero-generation swaps).** The stray report may contain `## Repair Block N` entries produced from the locked Positive-Constraint Document or the Name-Variant scan. For each one, perform a **literal substitution** in the draft — do NOT translate or paraphrase:
   ...
   - **Collision guard (MANDATORY — do not corrupt unrelated text):** apply a name swap ONLY where the `Find` form occurs as a **complete name token**. Specifically: never blind-swap a **single character** (it collides with ordinary words — swapping the name-syllable `申` for `森` would rewrite 森林/forest), and do NOT apply a swap at a position where the matched form is immediately followed/extended by another hanzi into a longer token (e.g. don't turn `薩拉曼德軍團` into `火蜥蜴軍團團` by matching only the `薩拉曼德軍` prefix). If a Repair Block looks unsafe to apply blindly (single char, or it would alter a longer surrounding token / a common word), DO NOT apply it — leave the text and note it for native-critique/operator review. You have document context; use it: a name fix must change names, never ordinary vocabulary.
