...
1.  **Stage A (Phases 2 & 3):** Process Chapter 1, then Chapter 2, and so on through the Extraction and Consolidation phases. This builds the `master_glossary.md` and narrative summaries incrementally.
...
2.  **Local Lexicographer (`local-lexicographer`)**
    ...
    - **Output**: `notes/<filename>.lexicon.md`.
...
1.  **Glossary Manager (`glossary-manager`)**
    ...
    - **Dependencies**: 
        - `local-lexicographer` (`notes/<original file>.lexicon.md`)
        - `metadata-generator` (`notes/metadata.md`)
        - Corresponding file in `original/` folder.
    - **Output**: Updates `notes/master_glossary.md`.

## 4. Production Phase (Multi-Stage Refinement)
**CRITICAL**: This phase must only be initiated after **ALL** sections have successfully completed the Extraction and Consolidation phases (1, 2, and 3). This ensures the `master_glossary.md` and all narrative summaries are fully finalized.
...
1.  **Primary Translator (`primary-translator`)**
    ...
    - **Dependencies**: 
        - **Global**: `master_glossary.md` (Final), `style_guide.md`, `metadata.md`, `contents.md`, and **all** `notes/*.summary.txt`.
...
1.  **Stray Phrase Detector (`stray-phrase-detector`)**
    ...
    - **Dependencies**: 
        ...
        - **Global**: `notes/metadata.md`.
    - **Output**: `notes/<filename>.stray_report.md`.

2.  **Stray Phrase Fixer (`stray-phrase-fixer`)**
    - **Description**: Translates identified stray phrases to ensure 100% coverage.
    - **Dependencies**: 
        - **Local**: `original/<filename>`, `draft/<filename>`, `notes/<filename>.stray_report.md`, and `notes/<filename>.summary.txt`.
        - **Global**: `master_glossary.md`, `style_guide.md`, `metadata.md`, `contents.md`.
...
1.  **Ebook Packager (`ebook-packager`)**
    ...
    - **Dependencies**: 
        - **Global**: `master_glossary.md`, `contents.md`, `metadata.md`.
