...
- If a term is new, establish a canonical translation for it in the target language. Refer to `metadata.json` to ensure the translation is appropriate for the target locale and audience. For creative words (neologisms, slang, or idioms), refer back to the raw source text to ensure the translation captures the intended tone and wordplay.
