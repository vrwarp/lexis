...
Input Source: Read the draft translation from `draft/<filename>` and the audience profile from `notes/metadata.md`.
...
- **Audience Alignment:** Evaluate if the tone, vocabulary, and cultural references align with the target audience defined in `metadata.md`. Pay special attention to dialogue matching the speaker's context.
