...
2. **Contextual Integration:** Ensure the fixed phrases match the surrounding grammar and tone of the existing draft.
3. **Adhere to Primary Constraints:**
    - **Target Locale & Audience:** Ensure the fix matches the dialect and level in `metadata.md`.
    - **Stylistic Alignment:** Match the author's voice as defined in `style_guide.md`.
    - **Lexical Consistency:** Use the `master_glossary.md` for all terms.
